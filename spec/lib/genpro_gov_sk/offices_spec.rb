...
RSpec.describe GenproGovSk::Offices, webmock: :disabled do
  describe '.import' do
    it 'fetches all 63 office URLs and calls import_from for each' do
      ...
      expected_urls_html =
        expected_urls.map do |url|
          "<tr class=\"govuk-table__row\"><td><a href=\"#{url.sub('https://www.genpro.gov.sk/', '')}\">Office Link</a></td></tr>"
        end

      allow(Curl).to receive(:get).with('https://www.genpro.gov.sk/kontakty-a-uradne-hodiny/').and_return(
        double(body_str: <<-HTML)
          <html>
            <body>
              <div class="tx-tempest-contacts">
                <table>
                  #{expected_urls_html.join("\n")}
                </table>
              </div>
            </body>
          </html>
      ...
      expected_urls.each { |url| allow(Curl).to receive(:get).with(url).and_return(double(body_str: '<html></html>')) }
